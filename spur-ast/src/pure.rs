...
pub enum PExprKind {
    ...
    VariantLit(NameId, NameId, Option<PAtomic>),
    IsVariant(PAtomic, NameId, NameId),
    ...
    FieldAccess(PAtomic, NameId),

    SafeFieldAccess(PAtomic, NameId),
    SafeIndex(PAtomic, PAtomic),
    SafeTupleAccess(PAtomic, usize),

    StructLit(NameId, Vec<(NameId, PAtomic)>),
