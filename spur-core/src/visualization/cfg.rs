...
fn generate_html_label(prog: &Program, _v: usize, label: &Label) -> (String, String) {
    ...
    match label {
        ...
        Label::PersistData(type_id, expr, _) => {
            header_color = "#DCEDC8"; // Light green
            content = format!(
                "<B>PersistData</B><BR/>type_id: {} {}",
                type_id.0,
                html_escape(&pretty_expr(prog, expr))
            );
        }
        Label::RetrieveData(type_id, lhs, _) => {
            header_color = "#DCEDC8"; // Light green
            content = format!(
                "<B>RetrieveData</B><BR/>type_id: {} {}",
                type_id.0,
                html_escape(&pretty_lhs(prog, lhs))
            );
        }
        ...
        Label::TraceDispatch(func_name, params, _) => {
            header_color = "#CE93D8"; // Deep purple
            let args_str: Vec<_> = params.iter().map(|a| pretty_expr(prog, a)).collect();
            content = format!(
                "<B>TraceDispatch</B><BR/>{}({})",
